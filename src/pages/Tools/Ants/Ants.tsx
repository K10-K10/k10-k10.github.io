...
function Ants() {
  ...
  const gridRef = useRef(Array.from({ length: SIZE }, () => Array(SIZE).fill(false)));
  ...
  return (
    <div className="ants">
      <Head
        title={"Langton's ant"}
        linkTitle={"Ants"}
        description={"K10-K10 site - Langton's ant simulator"}
      />
      ...
      <div className="ants-controls" style={{ marginTop: "10px" }}>
        <Button onClick={reset} disabled={false}>
          <Icon name="reset" theme="re" />
        </Button>
        <Button onClick={step} disabled={false}>
          <Icon name="jump" theme="re" />
        </Button>
